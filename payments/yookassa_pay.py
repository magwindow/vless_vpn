...
def create_payment(amount: float, user_id: int, tariff_key: str) -> str:
    payment = Payment.create({
        ...
        "confirmation": {"type": "redirect", "return_url": "https://t.me/vlessvpn24_bot"},
        "capture": True,
        "description": f"Оплата тарифа {tariff_key} пользователем {user_id}",
        "metadata": {"user_id": str(user_id)},
        "receipt": {
            "customer": {
                "email": "test@example.com"  # Замените на email пользователя
            },
            "tax_system_code": 1,  # Система налогообложения: 1 = ОСН (общая система)
            "items": [
                {
                    "description": f"Тариф {tariff_key}",
                    "quantity": 1.0,
                    "amount": {
                        "value": str(amount),
                        "currency": "RUB"
